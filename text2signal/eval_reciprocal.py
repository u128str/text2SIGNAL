...
from llm_eval_toolkit.data import UseCase, BaseEntry
...
from metric import  compute_score
...
subfolderGT=inputfolder
subfolder=inputfolder


@USE_CASE.register_input_file(name='input', subfolder=subfolderGT, pattern='*.json')
...
@USE_CASE.register_input_file(name='ground_truth', subfolder=subfolderGT, pattern='*.json')
...
def evaluation_func(entry: BaseEntry, prediction: Tuple[Any,Any]):
    """Function to compare prediction and ground_truth"""
...
if __name__ == '__main__':
    ...
    llm = init_llm(config['deployment_id'], temperature=config['temperature'], max_tokens=config['max_tokens'], )
    #print("STR llm",llm)
    predictor = LLMSignal2Text(llm, output_data_model=Signal2Text, config=config, verbose=True, use_openai_functions_api=False)
    ...
    # OUTPUT folder
    from llm_eval_toolkit.utils.callbacks.store_predictions import JSONLStorage, PickleStorage, JSONStorage
    date_str = datetime.datetime.now().strftime("%Y-%m-%dT")
    # base_folder / str('predictions-30-Nov-S2T-00-'+ str(config['deployment_id']))
    json_prediction_path= os.path.join(HERE, "predictions",  str(f'reciprocal-S2T-{date_str}_')+ str(config['deployment_id']) )
    # with inputfolder
