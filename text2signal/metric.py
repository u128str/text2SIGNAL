"""Compute the similarity between two dictionaries."""
import json
...
def dict_generator(indict, pre=None):
    """Generate all the keys and values from a dictionary."""
    pre = pre[:] if pre else []
...
def compute_score(gt, pr, model_name="multi-qa-MiniLM-L6-cos-v1"):
    ...
    # pr=pr1
    deep_diff = DeepDiff(
        gt, pr, ignore_order=False, cache_size=5000, get_deep_distance=True, ignore_private_variables=False
    )  # , view="tree")
    for el in linerized:
        ...
        if "values_changed" in deep_diff.to_dict().keys():
            if form_compatible_key in deep_diff.to_dict()["values_changed"]:
                ...
                if isinstance(new_value, str) and isinstance(old_value, str):
                    sm_simple = SequenceMatcher(None, new_value, old_value).ratio()
    ...
    for el in linerized:
        a = ".".join(
            [x for x in el[:-3] if not isinstance(x, int)]
        )  # remove int from full item address 'skills', 0, 'context' - > 'skills.context'
    ...
    list_of_lists.append(
        ["Embd.", res1["Avr. Score_Embd"].mean(), res1["Support_Embd"].sum(), res1["match_Embd"].sum()]
    )
    list_of_lists.append(
        ["Simple", res2["Avr. Score_Simple"].mean(), res2["Support_Simple"].sum(), res2["match_Simple"].sum()]
    )
